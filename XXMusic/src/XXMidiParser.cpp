...
void XX::Midi::Parser::processMessage(const std::vector<uchar>& message)
{
...
void XX::Midi::Parser::noteOn(const Channel& channel, const uchar& midiNote, const Velocity& velocity)
{
   for (const NoteOnFunction& noteOnFunction : std::as_const(noteOnFunctionList))
      noteOnFunction(channel, midiNote, velocity);
}

void XX::Midi::Parser::noteOff(const Channel& channel, const uchar& midiNote, const Velocity& velocity)
{
   for (const NoteOffFunction& noteOffFunction : std::as_const(noteOffFunctionList))
      noteOffFunction(channel, midiNote, velocity);
}

void XX::Midi::Parser::controllerChange(const Channel& channel, const ControllerMessage& controllerMessage, const uchar& value)
{
